...
class GradeLevelGroup extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final hasClassrooms = classrooms.isNotEmpty;
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(
          color: hasClassrooms ? Colors.transparent : Colors.grey.shade200,
        ),
        boxShadow: [
          BoxShadow(
        ...
        children: [
          ...
          Row(
            children: [
              ...
              if (!hasClassrooms) _buildGenerateButton(context, color),
              if (hasClassrooms)
                OutlinedButton(
                  onPressed: () => _showRegenerateDialog(context, controller),
          ...
          // Content based on whether classrooms exist
          if (hasClassrooms)
            ListView.builder(
              shrinkWrap: true,
            ...
            _buildEmptyState(context, color, controller),
  ...
  Widget _buildEmptyState(
    ...
    return Container(
      ...
      child: Column(
        children: [
          Icon(Icons.class_outlined, size: 48, color: color.withAlpha(50)),
  ...
  void _showRegenerateDialog(
    ...
    Get.dialog(
      AlertDialog(
        ...
        title: Text(
          'Regenerate ${_getGradeLevelDisplay(gradeLevel)} Classrooms?',
        ),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text(
              'This will delete all existing classrooms for ${_getGradeLevelDisplay(gradeLevel)} and create new ones.',
  ...
  void _showBulkSections(ClassroomController controller) {
    Get.dialog(
      AlertDialog(
        backgroundColor: TemarLijeColors.cardBackgroundColor,
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
                  ...
                  Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        SizedBox.shrink(),
                        // Text(
                        //   'Classroom: ${controller.selectedClassroom.value?.displayName}',
                        //   style: const TextStyle(fontWeight: FontWeight.bold),
                        // ),
                        const SizedBox(height: 4),
                        Text(
                          'Grade Level: ${controller.selectedClassroom.value!.gradeLevel.name.toTitleCase()}',
                          style: TextStyle(
                            fontSize: 12,
  ...
  void _deleteClassroom(Classroom classroom) async {
    final controller = Get.find<ClassroomController>();
    try {
      await controller.deleteClassroom(classroom);
    } catch (e) {
      Get.snackbar(
        'Error',
