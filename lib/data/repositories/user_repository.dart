...
class UsersRepository extends GetxService {
  ...
  Future<PrincipalModel> getPrincipal() async {
    try {
      final response = await _dioClient.get('/user/principal');

      // Parse the response into PrincipalModel model
      final principalData = PrincipalModel.fromJson(response.data);
  ...
  /// Returns created PrincipalModel object
  Future<PrincipalModel> createPrincipal(CreatePrincipalRequest request) async {
    try {
  ...
  /// Returns PrincipalModel object with user details
  Future<PrincipalModel> getPrincipals() async {
    try {
      final response = await _dioClient.get('/user/principal');
      final principalData = PrincipalModel.fromJson(response.data);
      return principalData;
  ...
  Future<List<PrincipalModel>> getAllPrincipals({
    int page = 1,
    int limit = 20,
  }) async {
    try {
      ...
      if (response.data is List) {
        return response.data
            .map<PrincipalModel>((json) => PrincipalModel.fromJson(json))
            .toList();
  ...
  /// Returns updated PrincipalModel object
  Future<PrincipalModel> updatePrincipal(
    ...
    try {
      ...
      final updatedPrincipal = PrincipalModel.fromJson(response.data);
  ...
  Future<PrincipalModel> patchPrincipal(
    String principalId,
  ...
  // Future<void> _cachePrincipalData(PrincipalModel principal) async {
  ...
  Future<PrincipalModel?> getCachedPrincipal({
    ...
    return PrincipalModel.fromJson(cachedData);
  }
